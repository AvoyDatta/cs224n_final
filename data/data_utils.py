...
class DJIA_Dataset(Dataset):
	def __init__(self, path_technical_csv, path_title_csv, randomize_sz=25,n=5,start=None,end=None):
		"""
		...
		self.technical_data = loadTechnical(path_technical_csv,n=n)
		print("technical data dims: ", self.technical_data.shape)
		self.targets, self.title_data = loadTitle(path_title_csv, randomize_sz=randomize_sz,n=n,start=start,end=end)
...
def loadTitle(input_csv_path, n=5, randomize_sz=None,start=None,end=None):
	"""
	...
	with tqdm(total=len(data) - n) as pbar:
		...
		for i in range(n, len(data)):
			...
			data_window = [data[j][2:] for j in range(i - n, i)]
...
if __name__ == "__main__":
	...
	n=5
	targets, new_tensor = loadTitle('Combined_News_DJIA.csv',n=n,start=start,end=end)
	print(new_tensor.shape)
	print(targets.shape)
	print(end-start-n)
# print(new_tensor.shape)
