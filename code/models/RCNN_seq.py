...
class RCNN_seq_2(nn.Module): 
	...
	def forward(self, titles, tech_indicators):
		...
		cat_sents = torch.cat((sents_h[0, :, :].squeeze(0), sents_h[1, :, :].squeeze(0), sents_c[0, :, :].squeeze(0), sents_c[1, :, :].squeeze(0)), 1) #(batch * window_len_days * num_titles_day, 4*n_filt_title)
		#print("sents cat:", cat_sents.shape)

		cat_sent_embeds = cat_sents.contiguous().view(batch_sz * self.config.window_len_titles, self.config.num_titles, 4 * self.config.n_filters_title) 
		#(batch * window_days,  num_titles, 4*n_hidden_sents)

		mapped_embeds = self.relu_title(self.map_down_embeds(cat_sent_embeds)) #(batch*window_days, num_titles, n_filt_title)


		mapped_embeds = mapped_embeds.permute(0, 2, 1) #(batch * window_days,  n_filt_title, num_titles)


		conv_out_day = self.conv_day(mapped_embeds) #Out: (batch * window_len_days, num_filters_day, num_titles_day - kernel_sz_day + 1)
