...
class RCNN_seq(nn.Module):
	def __init__(self, config):
		...
		self.map_to_out = nn.Linear(2 * config.num_LSTM_layers * config.n_hidden_LSTM, config.n_outputs)
	...
	def forward(self, titles, tech_indicators):
		...
		conv_out_titles = self.conv_title(titles) #Out: (batch, window_len_days, num_titles_day, num_filters_title, words_title - filter_sz_title + 1)
		# print(conv_out.shape)
		conv_title_pool_out = self.max_pool_title(conv_out) #Out: (batch, window_len_days, num_titles_day, num_filters_title, 1)
		# print(pool_out.shape)
		conv_title_pool_out = conv_title_pool_out.squeeze(-1) #Out: (batch, window_len_days, num_titles_day, num_filters_title)

		relud_pool_title = self.relu(conv_title_pool_out) #(batch, window_len_days, num_titles_day, num_filters_title)
		...
		# print(pool_out.shape)
		conv_day_pool_out = conv_title_pool_out.squeeze(-1) #Out: (batch, window_len_days, num_filters_day)

		relud_pool_day = self.relu(conv_day_pool_out) #Out: (batch, window_len_days, num_filters_day)
		# print(relud_pool.shape)
		# dropped_relu = self.dropout(relud_pool) #(batch, num_filters, L - R + 2) #NOT SURE ABOUT DIMENSION DROPPED
		# print(dropped_relu.shape)
		relud_pool_day_reshaped = relud_pool_day.permute(1, 0, 2) #Out: (window_len_days, batch, num_filters_day)

		concat_input = torch.cat((relud_pool_reshaped, tech_indicators), dim = 2) #Size: (window_len_days==seq_len_tech, batch, input_dim_tech + num_filters_day)
		...
		lstm_hidden_concat = torch.concat((last_hidden, last_cell), 1)

		end_states_concatenated = torch.concat((last_hidden, last_cell), dim = 1)

		output = self.log_softmax(self.map_to_out(end_states_concatenated)) #(batch, 2)
		# print(output.shape)
		return output
