...
class RCNN_seq_2(nn.Module):
	"""	
	RCNN_seq with LSTMs used to learn sentence embeddings	
	"""

	def __init__(self, config):
		super(RCNN_seq_2, self).__init__()
		self.config = config
		...
		self.lstm_titles = nn.LSTM(config.title_dim, config.n_filters_title, num_layers=2, batch_first=True)
		self.map_down_embeds = nn.Linear(4 * config.n_filters_title, config.n_filters_title)
		...
		self.map_down = nn.Linear(2 * config.num_LSTM_layers * config.n_hidden_LSTM,
		                          64)  # Maps down the concatenated hidden state inputs
	...
	def forward(self, titles, tech_indicators):
		...
		titles_reshaped = titles.contiguous().view(batch_sz * titles.size(1) * titles.size(2), titles.size(4),
		                                           titles.size(3))
		# (batch * window_len_days * num_titles_day, max_words, embed_size)
		# print("reshape initial:", titles_reshaped.shape)

		_, (sents_h, sents_c) = self.lstm_titles(titles_reshaped)  # sents_h: (2, new_batch, n_filt_title)
		# print("Sentence embeddings: ", sents_h.shape)
		cat_sents = torch.cat((sents_h[0, :, :].squeeze(0), sents_h[1, :, :].squeeze(0), sents_c[0, :, :].squeeze(0),
		                       sents_c[1, :, :].squeeze(0)),
		                      1)  # (batch * window_len_days * num_titles_day, 4*n_filt_title)
		# print("sents cat:", cat_sents.shape)

		cat_sent_embeds = cat_sents.contiguous().view(batch_sz * self.config.window_len_titles, self.config.num_titles,
		                                              4 * self.config.n_filters_title)
		# (batch * window_days,  num_titles, 4*n_hidden_sents)

		mapped_embeds = self.relu_title(
			self.map_down_embeds(cat_sent_embeds))  # (batch*window_days, num_titles, n_filt_title)

		mapped_embeds = mapped_embeds.permute(0, 2, 1)  # (batch * window_days,  n_filt_title, num_titles)

		conv_out_day = self.conv_day(
			mapped_embeds)  # Out: (batch * window_len_days, num_filters_day, num_titles_day - kernel_sz_day + 1)

		# print("Conv_out_day: ", conv_out_day.shape)

		conv_day_pool_out = self.max_pool_day(conv_out_day)  # Out: (batch * window_len_days, num_filters_day, 1)
		...
		concat_input = torch.cat((relud_pool_day_reshaped, tech_indicators),
		                         dim=2)  # Size: (window_len_days==seq_len_tech, batch, input_dim_tech + num_filters_day)

		# print("LSTM inputs: ", concat_input.shape)

		lstm_outputs, (last_hidden, last_cell) = self.lstm(
			concat_input)  # outputs shape: (seq_len, batch, hidden_sz). Both hidden & cell are (2, batch, hidden_size)
		...
		# print(self.attn_vector.data.grad)			# print(self.attn_vector.data.grad)
		# print("Attn proj: ", attn_proj.shape)			# print("Attn proj: ", attn_proj.shape)
...
class RCNN_seq(nn.Module):
	...
	def forward(self, titles, tech_indicators):
		...
		titles_reshaped = titles.contiguous().view(batch_sz * titles.size(1) * titles.size(2), titles.size(3),
		                                           titles.size(4))
		...
		conv_out_titles = self.conv_title(
			titles_reshaped)  # Out: (batch, window_len_days, num_titles_day, num_filters_title, words_title - filter_sz_title + 1)
		# print("conv_out_titles: ", conv_out_titles.shape)

		conv_title_pool_out = self.max_pool_title(
			conv_out_titles)  # Out: (batch * window_len_days * num_titles_day, num_filters_title, 1)

		conv_title_pool_out = conv_title_pool_out.contiguous().view(batch_sz, titles.size(1), titles.size(2),
		                                                            self.config.n_filters_title, 1)
		# (batch, window_len_days, num_titles_day, num_filters_title, 1)
		# print("reshape back # 1:", conv_title_pool_out.shape)

		conv_title_pool_out = conv_title_pool_out.squeeze(
			-1)  # Out: (batch, window_len_days, num_titles_day, num_filters_title)

		# print("Title pool: ",conv_title_pool_out.shape)

		relud_pool_title = self.relu_title(
			conv_title_pool_out)  # (batch, window_len_days, num_titles_day, num_filters_title)
		# ##print(relud_pool.shape)
		# dropped_relu = self.dropout(relud_pool) #(batch, num_filters, L - R + 2) #NOT SURE ABOUT DIMENSION DROPPED
		# ##print(dropped_relu.shape)
		relud_pool_title_reshaped = relud_pool_title.permute(0, 1, 3,
		                                                     2)  # (batch, window_len_days, num_filters_title, num_titles_day)

		# print(relud_pool_title_reshaped.shape)
		relud_pool_title_reshaped = relud_pool_title_reshaped.contiguous().view(
			batch_sz * self.config.window_len_titles,
			self.config.n_filters_title,
			self.config.num_titles)  # (batch * window_len_days, num_filters_title, num_titles_day)

		# print("Conv day input :", relud_pool_title_reshaped.shape)

		conv_out_day = self.conv_day(
			relud_pool_title_reshaped)  # Out: (batch * window_len_days, num_filters_day, num_titles_day - kernel_sz_day + 1)
...
class RCNN_seq_attn(nn.Module):
	...
	def forward(self, titles, tech_indicators):
		...
		output = self.log_softmax(self.map_to_out(mapped_down))  # (batch, 2)
		# print(output.shape)
		return output,self.attn_layer.weight
