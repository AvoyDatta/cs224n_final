...
def run_svm(data_train,data_extended_test,data_test):
	# train
	train_input = []
	train_labels = []
	...
	print("Loading validation (extended test) data...")
	...
	print("Validation (extended test) started...")
	# svc.fit(extended_test_input, extended_test_labels)
	print("Validation (extended test) finished... making predictions...")
	extended_test_pred = svc.predict(extended_test_input)
	print("Validation (extended test) accuracy: ", np.mean(extended_test_pred == extended_test_labels))
	...
	return train_accuracy,extended_test_accuracy,test_accuracy
...
if __name__ == "__main__":
	...
	for i in range(1,len(chunk_end_idxs)): #dummy for loop
		...
		max_idx_extended_test = max_idx_train + int(extended_test_split*len(chunk))
		max_idx_test = max_idx_extended_test + int(test_split*len(chunk))
		...
		data_extended_test = utils.data.Subset(chunk,[i for i in range(max_idx_train,max_idx_extended_test)])
		# print("data_extended_test length: ",len(data_extended_test))
		# dataloader_extended_test = DataLoader(data_extended_test,batch_size=int(batch_sz))

		data_test = utils.data.Subset(chunk, [i for i in range(max_idx_extended_test, max_idx_test)])
		# print("data_test length: ",len(data_test))

		# dataloader_test = DataLoader(data_test, batch_size = batch_sz)

		train_acc, extended_test_acc,test_acc = run_svm(data_train,data_extended_test,data_test)
	...
	print("avg extended test accuracy on windowed set: ",np.mean(chunk_extended_test_accs))
